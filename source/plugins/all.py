...
async def tgph(c, msg):
    ...
    await msg.reply_to_message.download("./VFF35")
    await msg.edit("جاري رفع الصورة علي تلجراف")
    response = upload_file("./VFF35")
    remove("./VFF35")
...
@Client.on_message(filters.command("ف", prefixes=f".") & filters.me)
...
@Client.on_message(filters.command("غ", prefixes=f".") & filters.me)
